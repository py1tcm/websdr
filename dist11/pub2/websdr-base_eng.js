     ...
     idle_page+='<div style="margin-bottom: 20px; background-color: #fff; border: 1px solid transparent; border-radius: 4px;">';
       idle_page+='<div style="padding: 25px;">';
         idle_page+='<div style="color: #ffffff; background-color: #ff0000; border-color: #ff0000; padding: 15px; border: 1px solid transparent; border-radius: 4px; text-align: center;" role="alert">';
           idle_page+='<span style="text-align: center; font-size: 24px;"><b>User inactivity.</b></span><br>';
           idle_page+='<span style="text-align: center; font-size: 18px;"><b>Idle time out. Time limit: '+ idle_sub_text +'</b></span>';
         idle_page+='</div>';
         idle_page+='<div style="border-radius: 4px; padding: 5px; margin: 8px 0 8px 0; border: solid #ffffff 1px; border: 1px solid transparent; border-radius: 4px; text-align: center;" role="alert">';
           idle_page+='<span style="text-align: center; font-size: 24px;">Dear visitor, in order to avoid the triggering of a timeout due to inactivity, we recommend you every '+ idle_sub_text +' perform any actions on the receiver, such as, for example, changing the frequency, volume level, range, etc. If the receiver is left without so-called supervision, it is automatically turned off, in order to ensure safety and save energy.</span><br>';
         idle_page+='</div>';
   ...
   for (b=0;b<nbands;b++) {
      if (!uu_compactview) {
	    ...
	    if (uu_names[i]=="WebSDR Volta Redonda") {
              ...
              s+='<button type="button" class="userbtn" onclick="setfreqb('+(uu_freqs[i]*bandinfo[b].samplerate+cbandfreq).toFixed(2)+');" title="Operator" style="cursor: pointer;margin:0; padding:0 4px; color:#ff3333;">'+uu_names[i]+'</button>';
              s+='</div>';
              total++;
 	    }
	    else {
...
//Hide java audio e java waterfall options (13/03/2021)
...
/*
   ...
   s='';
//   if (sup_iOS && sup_socket && sup_webaudio) s+='<input style="margin: 0 15px;" type="button" value=Audio start" onclick="iOS_audio_start()">';
//   if (sup_chrome && sup_socket && sup_webaudio) s+='<input style="margin: 0 15px;" type="button" value="Audio start" onclick="chrome_audio_start()">';
   if (sup_socket && sup_webaudio) s+='<input style="margin: 0 5px;" type="button" value="Audio start" onclick="chrome_audio_start()">';
...
function bodyonload()
{
   ...
   xhttp.onreadystatechange = function() 
     ...
     else { document.getElementById('geoip').value = ("Unknown location") }
     }
...

{
  var x= readCookie('username');
  if (x) {
    document.write('<span id="please"> Welcome! ');
    ...
    document.write('<span id="please"><span id="please1"><b><i>Please log in by typing your name or callsign here: <\/i><\/b></span> ');
...
//    time_out = time_out - parseInt(((samplecount - timeout_secs) % 3600) / 60);
    document.getElementById("conTime").innerHTML = windowhours + " hrs " + windowmins + " min " + windowsecs + " sec" // creates session time for display
    if (idletimeout){
